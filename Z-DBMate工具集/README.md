...
Z-DBMate 是华为 TPOPS + GaussDB 的自动化安装工具。本工具集在其基础上提供了 **预检、一键部署、进度监控、生产标准** 等配套能力，形成完整的部署流水线。
...
| 场景 | 配置文件 | 部署命令 |
|------|----------|---------|
| **单节点TPOPS + 三节点GaussDB** | `single_tpops_gaussdb_user_edit_file.conf` | `deploy.sh -s "单节点TPOPS+三节点GaussDB"` |
...
# 5. 回到迁移评估系统，执行正式迁移
#    使用 DRS/DataX 将数据从 Oracle 迁移到 GausDB
```
